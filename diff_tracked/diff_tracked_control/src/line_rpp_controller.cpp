...
public:
  ...
  bool Initialize() {
    ...
    odom_subscriber_ = nh_.subscribe(
        odom_topic_, 1, &RegulatedPurePursuitController::OdomCallback, this);

    control_timer_ =
        nh_.createTimer(ros::Duration(1.0 / control_rate_),
...
private:
  void LoadParameters() {
    ...
    private_nh_.param("max_lookahead_distance", max_lookahead_distance_, 1.0);
    ...
    private_nh_.param("planned_path_resolution", planned_path_resolution_,
                      0.25);
    private_nh_.param("actual_path_min_distance", actual_path_min_distance_,
                      0.05);
    private_nh_.param("actual_path_max_poses", actual_path_max_poses_, 5000);
  }
  ...
  bool ValidateParameters() const {
    ...
    if (!CheckPositive("min_lookahead_distance", min_lookahead_distance_)) {
      return false;
    }

    if (!CheckPositive("max_lookahead_distance", max_lookahead_distance_)) {
      return false;
    }

    if (!CheckPositive("regulated_min_radius", regulated_min_radius_)) {
      return false;
    }
    ...
    if (!CheckPositive("planned_path_resolution", planned_path_resolution_)) {
      return false;
    }

    if (!CheckPositive("actual_path_min_distance", actual_path_min_distance_)) {
      return false;
    }

    if (min_linear_speed_ < 0.0 || min_linear_speed_ > desired_speed_) {
  ...
  double YawFromQuaternion(const geometry_msgs::Quaternion &quaternion) const {
  ...
  void PublishPlannedPath(const nav_msgs::Odometry &odom) {
    ...
    for (int index = 0; index <= segment_count; ++index) {
      const double distance = target_distance_ * static_cast<double>(index) /
    ...
    ROS_INFO("Published %.1f m planned path with %zu poses", target_distance_,
             path.poses.size());
  }
  ...
  void ControlCallback(const ros::TimerEvent &event) {
    ...
    if (!odom_stamp.isZero()) {
      //信息年龄
      const double odom_age = (ros::Time::now() - odom_stamp).toSec();
    ...
    if (!event.last_real.isZero()) {
      //
      control_period = (event.current_real - event.last_real).toSec();
    }
    ...
    const double remaining = target_distance_ - along_track;

    const double target_x = start_x_ + target_distance_ * std::cos(start_yaw_);

    const double target_y = start_y_ + target_distance_ * std::sin(start_yaw_);
    ...
    if (goal_distance <= distance_tolerance_ && std::abs(cross_track) <= 0.20) {
      finished_ = true;
      PublishStop();
    ...
    const double measured_speed = std::abs(latest_odom_.twist.twist.linear.x);

    double lookahead = lookahead_distance_ + lookahead_time_ * measured_speed;

    lookahead =
        Clamp(lookahead, min_lookahead_distance_, max_lookahead_distance_);
    //计算前视点在线路上的距离
    const double carrot_distance =
        Clamp(along_track + lookahead, 0.0, target_distance_);
    //前视点二维坐标
    const double carrot_x = start_x_ + carrot_distance * std::cos(start_yaw_);
    const double carrot_y = start_y_ + carrot_distance * std::sin(start_yaw_);
    ...
    const double carrot_robot_distance_squared =
        carrot_robot_x * carrot_robot_x + carrot_robot_y * carrot_robot_y;

    double curvature = 0.0;
    const double curvature_epsilon = 1.0e-8;

    if (carrot_robot_distance_squared > curvature_epsilon) {
      curvature = 2.0 * carrot_robot_y / carrot_robot_distance_squared;
    }
    ...
    if (absolute_curvature > curvature_epsilon) {
      const double turning_radius = 1.0 / absolute_curvature;

      if (turning_radius < regulated_min_radius_) {
        target_linear_speed = turning_radius / regulated_min_radius_;
      }
      ...
      //两个限制
      target_linear_speed =
          std::min(target_linear_speed, lateral_acceleration_speed);
    }
    ...
    target_linear_speed =
        Clamp(target_linear_speed, min_linear_speed_, desired_speed_);

    if (absolute_curvature > curvature_epsilon) {
      target_linear_speed = std::min(target_linear_speed,
                                     max_angular_speed_ / absolute_curvature);
    }
    //
    const double linear_acceleration =
        target_linear_speed >= last_linear_command_ ? max_linear_accel_
                                                    : max_linear_decel_;
    //最多变化指定步长
    const double linear_command =
        Approach(last_linear_command_, target_linear_speed,
                 linear_acceleration * control_period);

    const double target_angular_command = Clamp(
        linear_command * curvature, -max_angular_speed_, max_angular_speed_);

    const double angular_command =
        Approach(last_angular_command_, target_angular_command,
    ...
    ROS_INFO_THROTTLE(1.0,
                      ...
                      "cmd (%.2f m/s, %.2f rad/s)",
                      along_track, target_distance_, cross_track, lookahead,
                      curvature, linear_command, angular_command);
  }
